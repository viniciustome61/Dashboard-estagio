...
async function iniciarDashboard() {
    try {
        ...
        iniciarRotacao(120000); // 2 minutos de intervalo
...
// --- Funções da Tela de Contratos (CORRIGIDA) ---
...
function renderizarPainelContratos(dadosContratos) {
    ...
    contratosValidos.forEach((contrato, index) => {
        ...
        slide.addEventListener('click', () => {
            ...
            // CORREÇÃO: Usando os nomes exatos das colunas do novo CSV
            ...
            preencherDetalhe('detalhe-contrato-prorrogacao1', contrato['PRORROGAÇÃO 1']);
            preencherDetalhe('detalhe-contrato-prorrogacao2', contrato['PRORROGAÇÃO 2']);
            preencherDetalhe('detalhe-contrato-repactuacao1', contrato['REPACTUAÇÃO 1']);
            preencherDetalhe('detalhe-contrato-repactuacao2', contrato['REPACTUAÇÃO 2']);
            preencherDetalhe('detalhe-contrato-responsavel', contrato['RESPONSÁVEL CONTRATADA']);
            ...
            // CORREÇÃO: Usando a coluna 'PROCESSO RAIZ'
            const processoUrl = contrato['PROCESSO RAIZ']; 

            if (processoUrl && processoUrl.trim() !== '') {
                spanProcesso.innerHTML = `<a href="${processoUrl}" target="_blank" rel="noopener noreferrer">Acessar Processo</a>`;
                itemProcesso.style.display = 'list-item';
            } else {
                spanProcesso.textContent = 'Não informado';
                itemProcesso.style.display = 'list-item';
            }
